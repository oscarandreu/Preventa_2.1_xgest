...
public class GuiPedidos extends GuiInternalFrame {
	...
	JTextField jTaño = null;

	JTextField jTnumeropedido = null;
	...
	JTextField jTHora = null;

// ==================================================================
	public GuiPedidos(Cargador cargador)
// ==================================================================
	{
		super(cargador);
	...
	// ==================================================================
	{
		if (panel == null) {
			...
			filasImportadas.setText("Numero de Nuevos Pedidos : "
					+ model.numeroFilas);
		}
		return panel;
	}
	...
	// ===========================================================================================
	{
		...
		pedidos.setHorizontalScrollBarPolicy(ScrollPaneConstants.HORIZONTAL_SCROLLBAR_ALWAYS);
...
// ===========================================================================================
		...
		try {
			...
			if (resul.next()) {
				...
				if (totalConIva == 0) {
					...
					double tempNet =  calcularTotalNeto( jTnumeropedido.getText(),serie.getText(),SysData.formatYearToFirebird(jTaño.getText()));
							...
							if ( n == 0 ){
								...
								*/
								...
								query += " WHERE codigo_cliente='"+jTcodigo.getText()+"'" +
										" AND numero_cabecera='" + jTnumeropedido.getText() +"'" +
	...
	private JButton getValidar() {
		if (validar == null) {
			...
			validar.setBounds(new java.awt.Rectangle(50,235,124,35));
						...
						try {
							...
							if (n == 0) {
								...
								case 1:
									...
									cabErp.insertarPedidoERP( s,st,serie.getText(),SysData.formatYearToFirebird(jTaño.getText()),jTnumeropedido.getText(),jTcodigo.getText());
									s.close();
									st.close();
									break;
								case 2:
							...
							new InfoPane("ERROR","Se ha producido un ERROR al Validar el Pedido");
						}
						cargador.actualizarVentanas();
					}
				}
...
//  ======================================================================	
	public double calcularTotalNeto(String cabec, String serie, String anyo)
//	======================================================================
	{
		double res = 0;
		...
		try{
			...
			ResultSet rs = st.executeQuery( "SELECT TOTAL " +
					"FROM linea_pedido " +
					"WHERE numero_cabecera = '" + cabec + "' AND serie='" + serie + "' AND anyo=" + anyo);
			while ( rs.next() ){
				double totlin = rs.getDouble("total");
				double tmp = totlin * (dto1/100);
				totlin = totlin - tmp;
